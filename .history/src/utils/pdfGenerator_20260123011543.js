...
export const generatePDF = async () => {
  ...
  try {
    ...
    // CONTACT INFORMATION - Two columns with icons
    pdf.setFontSize(10);
    pdf.setTextColor(textDark[0], textDark[1], textDark[2]);
    
    // Left column
    pdf.text(`✉ ${resumeData.personal.email}`, margin, currentY);
    pdf.text(`📍 ${resumeData.personal.location}`, margin, currentY + 5);
    
    // Right column
    pdf.text(`📱 ${resumeData.personal.phone}`, margin + 100, currentY);
    pdf.text(`💼 ${resumeData.contact.linkedin.replace('https://www.', '')}`, margin + 100, currentY + 5);
