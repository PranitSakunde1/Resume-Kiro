    ...
    <section id="experience" className="py-20 bg-gradient-to-br from-white via-gray-50 to-primary-50 dark:from-dark-900 dark:via-dark-800 dark:to-dark-900 relative overflow-hidden">
      ...
      <div className="container-max section-padding relative z-10">
        ...
        <div className="relative max-w-6xl mx-auto">
          {/* Enhanced modern timeline line */}
          <div className="absolute left-4 md:left-1/2 transform md:-translate-x-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-primary-200 via-primary-400 to-primary-600 dark:from-primary-800 dark:via-primary-600 dark:to-primary-400 opacity-30"></div>
            ...
            {experience.map((job, index) => (
              ...
              >
                ...
                <div className="absolute left-4 md:left-1/2 transform md:-translate-x-1/2 -translate-y-1 z-20">
                  <div className="relative timeline-indicator">
                    {/* Outer glow ring */}
                    <div className={`w-8 h-8 rounded-full bg-gradient-to-r ${job.type === 'current' ? 'from-green-400 to-emerald-500' : 'from-primary-400 to-primary-600'} opacity-20 animate-ping`}></div>
                    
                    {/* Main indicator */}
                    <div className={`absolute inset-1 w-6 h-6 rounded-full bg-gradient-to-r ${job.type === 'current' ? 'from-green-500 to-emerald-600' : 'from-primary-500 to-primary-700'} shadow-lg flex items-center justify-center hover:shadow-xl transition-shadow duration-300`}>
                      {/* Inner dot with subtle animation */}
                      <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
                    </div>
                    
                    {/* Status indicator for current role */}
                    {job.type === 'current' && (
                      <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-400 rounded-full animate-pulse border-2 border-white dark:border-dark-900 shadow-lg"></div>
                    )}
                    
                    {/* Hover effect ring */}
                    <div className={`absolute inset-0 w-8 h-8 rounded-full border-2 ${job.type === 'current' ? 'border-green-400' : 'border-primary-400'} opacity-0 hover:opacity-50 scale-150 hover:scale-200 transition-all duration-300`}></div>
                  </div>
                </div>

                {/* Content */}
                <div className={`flex-1 ml-16 md:ml-0 ${index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12 md:ml-auto'}`}>
                  <div className="group bg-white dark:bg-dark-900 rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 hover:transform hover:scale-105 border border-gray-100 dark:border-dark-700 relative overflow-hidden">
                    
                    {/* Modern gradient overlay */}
                    <div className="absolute inset-0 bg-gradient-to-br from-primary-500/3 via-transparent to-purple-500/3 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    
                    {/* Decorative corner element */}
                    <div className={`absolute top-0 ${index % 2 === 0 ? 'right-0' : 'left-0'} w-20 h-20 bg-gradient-to-br from-primary-400/10 to-purple-400/10 rounded-full -translate-y-10 ${index % 2 === 0 ? 'translate-x-10' : '-translate-x-10'} group-hover:scale-150 transition-transform duration-700`}></div>
                ...
                {/* Enhanced modern years indicator */}
                <div className={`hidden md:block ${index % 2 === 0 ? 'order-2' : 'order-1'}`}>
                  <div className="relative group">
                    {/* Background with gradient */}
                    <div className="absolute inset-0 bg-gradient-to-r from-primary-500 to-primary-600 rounded-2xl blur-sm opacity-20 group-hover:opacity-30 transition-opacity"></div>
                    
                    {/* Main year badge */}
                    <div className="relative bg-white dark:bg-dark-800 border border-primary-200 dark:border-primary-800 px-6 py-4 rounded-2xl font-bold text-lg whitespace-nowrap shadow-lg hover:shadow-xl transition-all duration-300 hover:transform hover:scale-105">
                      <div className="text-primary-600 dark:text-primary-400 text-center">
                        <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Started</div>
                        <div className="text-lg font-bold">{job.duration.split(' - ')[0]}</div>
                      </div>
                      
                      {/* Status indicator */}
                      {job.type === 'current' && (
                        <div className="absolute -top-2 -right-2 px-2 py-1 bg-green-500 text-white text-xs font-bold rounded-full shadow-lg animate-pulse">
                          Now
                        </div>
                      )}
                    </div>
